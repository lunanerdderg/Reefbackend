...
bool cli(bool devMode) {
    ...
    while (running) {
        ...
        // ------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        else if (command.substr(0,14) == "getAsyncToCout" && devMode) { // BOOKMARK // Get browser_download_url // getAsyncToCout "https://api.github.com/repos/toebeann/BepInEx.Subnautica/releases/latest"
            index = readTo(command, '"', 16)-1;
            std::string location = command.substr(16, index - 15);
            cpr::AsyncResponse fr = cpr::GetAsync(cpr::Url{location});
            ...
            std::ofstream foutOne("sample.zip", std::ios::binary);
            foutOne << r.text;
            foutOne.close();
            std::cout << "Success!\n";
        }
